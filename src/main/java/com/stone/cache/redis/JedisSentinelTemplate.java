 ...
 */
public class JedisSentinelTemplate implements InitializingBean,Serializable{

    private static final Logger logger = LoggerFactory.getLogger(JedisSentinelTemplate.class);

    private boolean initialized = false;
    private boolean enableDefaultSerializer = true;
    private JedisSentinelPool firstPool;
    ...
    //服务器节点
    private Map<Integer, JedisSentinelPool> shards = new HashMap<Integer, JedisSentinelPool>();
    ...
    public void afterPropertiesSet() {
        ...
        if(Integer.bitCount(shardsCount) == 1){
            isHash = true;
        }
        initialized = true;
    }
    ...
    public Jedis getJedis(String key){
        return getJedisSentinelPool(key).getResource();
    }
    ...
    private int doFindIndex(String key, int shardsCount){
        if(isHash){
            int h = hash(key);
            //key 值从 1 开始
            return (1 + indexFor(h, shardsCount));
        }else{
            int h = hash3(key);
            //key 值从 1 开始
            return (1 + indexFor3(h, shardsCount));
        }
    }
    /**
     *
    ...
    public JedisSentinelPool getJedisSentinelPool(String key) {
        ...
        try {
            //key 值从 1 开始
            int index = doFindIndex(key, shardsCount);
            logger.debug(">>> -- choice pool[{}] ,key[{}]", index, key);
     ...
     * @return firstPool
     */
    private JedisSentinelPool getFirstPool(){
        if(firstPool != null){
     ...
     * @param firstPool
     */
    public void setFirstPool(JedisSentinelPool firstPool) {
    ...
    public Map<Integer, JedisSentinelPool> getShards() {
        return shards;
    }

    public void setShards(Map<Integer, JedisSentinelPool> shards) {
        this.shards = shards;
    }
